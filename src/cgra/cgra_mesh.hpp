...
namespace cgra {
	...
	struct gl_mesh {
		...
		// calls the draw function for Instances
		void drawInstances();
	...
	//	mesh_builder_boundingBox() {}

		void getMaxgetMin(glm::vec3 &maxV, glm::vec3&minV) {
			for (mesh_vertex m: vertices) {
				glm::vec3 v = m.pos;
				//Find Minimum Verticies
				if (v.x < minV.x) {
					minV.x = v.x;
				}
				if (v.y < minV.y) {
